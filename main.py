...
CRYPTO = [ "BTC-USD", "ETH-USD", "BNB-USD", "SOL-USD", "XRP-USD", "ADA-USD", "DOGE-USD", "DOT-USD",
...
STOCKS = [ "AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "META", "TSLA", "TSM", "JPM", "BAC",
         "UNH", "JNJ", "WMT", "HD", "XOM", "CVX", "AMD", "INTC", "ASML", "SMCI", "AVGO",
         "ARM", "LLY", "NVO", "COST", "PANW", "CRWD", "UBER"]
COMMODITIES = ["GC=F", "SI=F", "CL=F", "NG=F", "ZC=F", "ZW=F", "ZS=F", "ES=F", "NQ=F",
...
class PremiumTradingBot:
    ...
    async def analyze_and_notify(self):
        ...
        print(f"\nსკანირება: {len(ASSETS)} აქტივი")
        print(f"აქტიური Subscribers: {active_count}\n")
        processed = 0
        for asset in ASSETS:
            processed += 1
            print(f"[{processed}/{len(ASSETS)}] {asset}...")
            ...
            if asset in self.active_positions:
            ...
            elif data['price'] > data['ema200'] and (data['rsi'] < 38 or data['price'] <= data['bb_low']):
                ...
                if is_clean:
                    ...
                    stop_loss_price = data['price'] * (1 - STOP_LOSS_PERCENT / 100)
                    take_profit_price = data['price'] * (1 + TAKE_PROFIT_PERCENT / 100)
                    msg = f"🟢 იყიდე: {asset} [{asset_type}]\n\n💵 ფასი: ${data['price']:.2f}\n📊 RSI: {data['rsi']:.1f}\n📈 EMA200: ${data['ema200']:.2f}\n\n🎯 Risk Management:\n🔴 Stop-Loss: ${stop_loss_price:.2f} (-{STOP_LOSS_PERCENT}%)\n🟢 Take-Profit: ${take_profit_price:.2f} (+{TAKE_PROFIT_PERCENT}%)\n⏰ Max Hold: {MAX_HOLD_HOURS}ს\n\n💡 მიზეზი: ფასი ინარჩუნებს ზრდის ტრენდს და ამჟამად ხელსაყრელ დაბალ წერტილზეა."
                    await self.broadcast_signal(msg, asset)
                else:
                    print(f"ნეგატიური ნიუსების გამო დაბლოკილია")
            await asyncio.sleep(ASSET_DELAY)
        cycle_duration = time.time() - cycle_start
        print(f"\nციკლი დასრულდა: {cycle_duration/60:.1f}წთ")
    ...
    async def check_exit_conditions(self, asset, data):
        ...
        elif profit_percent < -3 and data['rsi'] < 30:
            should_exit = True
        ...
        elif data['rsi'] > 65 or data['price'] >= data['bb_high']:
            should_exit = True
            exit_reason = "📈 ტექნიკური სიგნალი (RSI/BB)"
        if should_exit:
            balance_1usd = 1.0 * (1 + (profit_percent / 100))
            asset_type = self.get_asset_type(asset)
            emoji = "🔴" if profit_percent < 0 else "🟢"
            msg = f"{emoji} გაყიდე: {asset} [{asset_type}]\n\n📊 შესვლა: ${entry_price:.2f}\n📊 გასვლა: ${current_price:.2f}\n💰 მოგება: {profit_percent:+.2f}%\n💵 1$-ის ბალანსი: ${balance_1usd:.4f}\n\n📌 მიზეზი: {exit_reason}"
            del self.active_positions[asset]
            await self.broadcast_signal(msg, asset)
            print(f"SELL სიგნალი - მოგება: {profit_percent:.2f}%")
    ...
    async def start(self):
        ...
        startup_msg = f"💎 Premium Trading Bot ჩაირთო\n\n📊 მონიტორინგი:\n🔸 {len(CRYPTO)} კრიპტოვალუტა\n🔸 {len(STOCKS)} აქცია\n🔸 {len(COMMODITIES)} საქონელი\n━━━━━━━━━━━━━━━━\n📡 სულ: {len(ASSETS)} აქტივი\n⏱️ ციკლი: ~{len(ASSETS) * ASSET_DELAY / 60:.0f}წთ\n👥 Subscribers: {len(self.subscriptions)}"
        ...
        while True:
            ...
            print(f"შესვენება {SCAN_INTERVAL}წმ...\n")
            await asyncio.sleep(SCAN_INTERVAL)

if __name__ == "__main__":
    print("🚀 ბოტი იწყება...\n")
    bot = PremiumTradingBot()
