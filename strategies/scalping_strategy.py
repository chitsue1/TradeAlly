...
class ScalpingStrategy(BaseStrategy):
    ...
    def __init__(self):
        ...
        # Configuration
        self.min_cooldown_hours = 6  # 6h cooldown prevents re-entry on same symbol same session
        self.min_confidence = 55.0  # P2/#8
    ...
    def analyze(
        ...
        signal = TradingSignal(
            ...
            expected_hold_duration=f"15-40 min (signal sent {datetime.now().strftime('%H:%M')})",
            ...
            timeframe_context={
                ...
                'signal_sent_at':    datetime.now().isoformat(),
                'entry_valid_until': (datetime.now() + timedelta(minutes=60)).strftime('%H:%M'),
            }
        )
